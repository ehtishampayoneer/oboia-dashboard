...
// ─────────────────────────────────────────────────────────────────────────
// Self-contained bilingual strings + local language toggle. This page does
// NOT depend on LanguageContext on purpose — signup is a public page that
// must work even if a provider isn't mounted, and a missing context value
// was a likely cause of the silent "button does nothing" failure.
// ─────────────────────────────────────────────────────────────────────────
const STRINGS = {
  en: {
    ...
    sign_in: 'Sign In',
    err_required: 'Please fill in shop name, email and password.',
  ...
  uz: {
    ...
    sign_in: 'Kirish',
    err_required: 'Do\'kon nomi, email va parolni to\'ldiring.',
...
export default function SignupPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Surface the REAL reason — never fail silently.
      ...
      if (err?.code === 'auth/email-already-in-use') {
        ...
        setErrorMsg(`Error: ${err?.code || err?.message || 'unknown'}`);
      }
    } finally {
      setSubmitting(false);
    }
