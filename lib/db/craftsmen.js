...
// ─────────────────────────────────────────────────────────────────────────
// DELETE a craftsman — permanently removes the document.
// BUSINESS RULE: refuse if there is a pending (unpaid) bonus balance, so we
// never erase an obligation we still owe. Pay it out first, then delete.
// Throws 'HAS_PENDING_BALANCE' so the UI can show the right message.
// ─────────────────────────────────────────────────────────────────────────
export async function deleteCraftsman(id) {
  const ref = doc(db, 'craftsmen', id);
  const snap = await getDoc(ref);
  if (!snap.exists()) throw new Error('Craftsman not found');
  const data = snap.data();
  if ((data.pendingBalance || 0) > 0) {
    throw new Error('HAS_PENDING_BALANCE');
  }
  await deleteDoc(ref);
}
/**
